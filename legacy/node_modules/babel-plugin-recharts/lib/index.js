...
function findPath(source) {
  ...
  var _Module$_resolveLooku = _module3.default._resolveLookupPaths(source, finalModule),
      _Module$_resolveLooku2 = _slicedToArray(_Module$_resolveLooku, 2),
      id = _Module$_resolveLooku2[0],
      paths = _Module$_resolveLooku2[1];

  var finalPaths = [].concat(_toConsumableArray(paths), [rechartsSrcPath]);

  var sourceFullPath = void 0;
  try {
    var fullPath = _path2.default.resolve(rechartsSrcPath, source.indexOf('.js') >= 0 ? source : source + '.js');
  ...
  if (sourceFullPath) {
    ...
    // 完整引用路径
    var sourceLibPath = rechartsLib + '/' + _path2.default.relative(rechartsSrcPath, sourceFullPath);
    return sourceLibPath;
  }

  var absPath = _module3.default._findPath(source, finalPaths);

  if (absPath && absPath.indexOf(_path2.default.join(rechartsPath, 'node_modules')) >= 0 || absPath.indexOf('node_modules') >= 0) {
    // node_modules source
    return source;
  }
